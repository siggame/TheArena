...
namespace TheArena
{
    ...
    {
        /// <summary>
        /// Installs LuaJit, LuaRocks, and LuaSocket on Ubuntu needed to run the Lua Joueur Client
        ...
        /// <summary>
        /// Given the file path, compile the AI and run it using Lua -- run until the results file shows a win, loss, or error
        /// </summary>
                ...
                using (Process process = new Process())
                {
                    ...
                    if (process.Start())
                    {
                        ...
                        using (StreamWriter sw = new StreamWriter(file.Substring(0, file.LastIndexOf('/') + 1) + "testRun"))
                        {
                            ...
                            sw.WriteLine("  else");
                            sw.WriteLine("    ./run "+DA_GAME+" -s game.siggame.io -r \"$@\"");
                            sw.WriteLine("fi");
                        }
